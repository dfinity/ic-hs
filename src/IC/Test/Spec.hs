...
icTests my_sub other_sub =
  ...
  , testGroup "uninstall"
    ...
    , simpleTestCase "uninstalled rejects calls" ecid $ \cid -> do
      ...
      call'' cid (replyData "Hi") >>= isErrOrReject [3]
  ...
  , testGroup "canister_inspect_message"
    [ testCase "empty canister" $ do
      cid <- create ecid
      call'' cid reply >>= isErrOrReject [3]
      callToQuery'' cid reply >>= isErrOrReject [3]
    ...
    , testCase "no accept_message" $ do
      ...
      call'' cid reply >>= isErrOrReject [4]
      callToQuery'' cid reply >>= isErrOrReject [4]
    ...
    , testCase "two calls to accept_message" $ do
      ...
      callToQuery'' cid reply >>= isErrOrReject [5]
